...
public class DbAdapter {
	...
	public long createrecord(String distance, String duration,
			String averagespeed, String pathline, String records, String stratpoint,
	...
	public List<PathRecord> queryRecordAll() {
		...
		while (allRecordCursor.moveToNext()) {
			...
			String records = allRecordCursor.getString(allRecordCursor
					.getColumnIndex(DbAdapter.KEY_LINE));
			record.setmPathRecordPoints(Util.parseLocations(records));
	...
	public PathRecord queryRecordById(int mRecordItemId) {
		...
		if (cursor.moveToNext()) {
			...
			String mRecords = cursor.getString(cursor
					.getColumnIndex(DbAdapter.KEY_RECORDS));
			record.setmPathRecordPoints(Util.parseLocations(mRecords));
	...
	private String[] getColumns() {
		return new String[] { KEY_ROWID, KEY_DISTANCE, KEY_DURATION, KEY_SPEED,
				KEY_LINE, KEY_RECORDS, KEY_STRAT, KEY_END, KEY_DATE };
	}
}
